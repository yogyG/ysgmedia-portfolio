(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))s(o);new MutationObserver(o=>{for(const t of o)if(t.type==="childList")for(const n of t.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function a(o){const t={};return o.integrity&&(t.integrity=o.integrity),o.referrerPolicy&&(t.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?t.credentials="include":o.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function s(o){if(o.ep)return;o.ep=!0;const t=a(o);fetch(o.href,t)}})();function y(e){return`
  ...
  `}function b(e){return`
    <div class="card-glass hover-lift" style="padding: var(--spacing-md); border-radius: var(--border-radius-md); background: var(--bg-card); border: 1px solid var(--border-muted);">
      ...
      <div style="display: flex; align-items: center; gap: var(--spacing-sm); margin-top: var(--spacing-sm);">
        <img src="${e.avatarUrl||"assets/images/logos/placeholder_avatar.png"}" alt="Avatar" style="width: 48px; height: 48px; border-radius: 50%; object-fit: cover; border: 2px solid var(--bg-primary);" />
        <div style="font-weight: 600; font-size: 0.9rem; color: var(--text-primary);">${e.clientName}</div>
      </div>
    </div>
  `}function x(e,r="home"){const s=[{name:"Home",url:"index.html"},{name:"Services",url:"services.html"},{name:"Case Studies",url:"case-studies.html"},{name:"Blog",url:"blog.html"},{name:"About",url:"about.html"},{name:"Contact",url:"contact.html"}].map(t=>{const n=r===t.name.toLowerCase().replace(" ","-")||r==="home"&&t.url==="index.html";return`<a href="${t.url}" class="${n?"active":""}">${t.name}</a>`}).join(""),o="https://wa.me/message/TES4XUVEGSGBC1";return`
    <header class="global-header">
      ...
      <nav class="nav-menu" id="nav-menu">
        ...
        <div class="mobile-actions">
          ...
          <a href="${o}" target="_blank" class="btn-whatsapp">
  ...
  `}document.addEventListener("DOMContentLoaded",()=>{v(),g(),p(),h(),m()});function m(){const e=document.getElementById("mobile-hamburger"),r=document.getElementById("nav-menu");e&&r&&(e.addEventListener("click",a=>{a.stopPropagation(),r.classList.toggle("active"),e.classList.toggle("active")}),document.addEventListener("click",a=>{!r.contains(a.target)&&!e.contains(a.target)&&(r.classList.remove("active"),e.classList.remove("active"))}))}function v(){const e=document.querySelectorAll(".reveal-on-scroll"),r={root:null,threshold:.15,rootMargin:"0px 0px -50px 0px"},a=new IntersectionObserver((s,o)=>{s.forEach(t=>{t.isIntersecting&&(t.target.classList.add("revealed"),o.unobserve(t.target))})},r);e.forEach(s=>{a.observe(s)})}function g(){const e=document.querySelector(".hero-dynamic-word");if(!e)return;const r=JSON.parse(e.getAttribute("data-words")||'["Leads", "Calls", "Customers"]');let a=0;setInterval(()=>{e.style.opacity=0,e.style.transform="translateY(10px)",setTimeout(()=>{a=(a+1)%r.length,e.textContent=r[a],e.style.opacity=1,e.style.transform="translateY(0)"},300)},3e3)}function p(){const e=document.querySelectorAll(".stat-counter");if(e.length===0)return;const r=new IntersectionObserver((a,s)=>{a.forEach(o=>{if(o.isIntersecting){const t=o.target,n=parseInt(t.getAttribute("data-target")||"0",10),l=t.getAttribute("data-suffix")||"",c=Math.max(Math.floor(2e3/n),15);let i=0;const d=setInterval(()=>{i+=1,t.textContent=i+l,i>=n&&(t.textContent=n+l,clearInterval(d))},c);s.unobserve(t)}})},{threshold:.5});e.forEach(a=>{r.observe(a)})}function h(){const e=document.getElementById("dashboard-chart");if(!e)return;new IntersectionObserver((a,s)=>{a.forEach(o=>{if(o.isIntersecting){const t=e.querySelector(".animate-path");t&&(t.style.animationPlayState="running"),s.unobserve(e)}})},{threshold:.2}).observe(e)}export{w as a,y as b,f as c,b as d,x as g};
